...
if(isset($_POST['search'])) //search input form user
{
$c_id=$_POST['c_id'];
$sql="SELECT * FROM finalized_quotes WHERE c_id='$c_id'";  //search in SQL
$result=mysqli_query($conn,$sql);
 if(mysqli_num_rows($result)>0)
 {
  while($row=mysqli_fetch_assoc($result)) //fetch the data
 {

          echo '<tr style="color:white;">';  //show the table with reults
          echo '<td>'.$row['q_id'].'</td>';
